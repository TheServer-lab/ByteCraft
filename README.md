...
Overwrites a file unconditionally. Unlike `make-file`, no warning is emitted if the file already exists — not even in `strict on` mode. Use this when overwriting is the intended behaviour and you want the script to stay clean.
...
# Multi-line content block
replace-file "config/settings.py" with ---
DEBUG = False
...
Runs a separate `.bc` script with a **completely clean state** — no variables, no templates, no working folder carried over from the calling script. Use this to split large scaffolds across multiple files without hitting file size limits. The calling script continues normally after `start-new` finishes, with its own state unchanged.

Supports both local paths and remote URLs. If the file is missing or the URL fails, execution halts with a fatal error.

```
# Local file
start-new "part2.bc"
start-new "scaffolds/database.bc"

# Remote URL
start-new "https://example.com/shared-scaffold.bc"
```

```
[Bytecraft:5] Starting new scaffold from: /path/to/part2.bc
...
[Bytecraft:5] Starting new scaffold from remote: https://example.com/shared-scaffold.bc
```

Local paths resolve relative to the calling script's directory. Remote scripts resolve relative paths from the current working directory, the same as running them directly from the CLI.
...
- [x] `--dry-run` CLI flag — now fully side-effect free (no directories created on disk)
- [x] Line numbers in all log output
- [x] `--help` / `-h` — full language reference in the terminal
- [x] `--version` / `-v`
- [x] Overwrite warning on `make-file` (fatal in strict mode)
- [x] `replace-file` — unconditional overwrite, silent in strict mode
- [x] `extract` — extract full zip contents to a folder (merge/overwrite if destination exists)
- [x] `extract-file` — extract a single file from a zip by its internal path
- [x] Remote script execution — `bytecraft https://example.com/script.bc` fetches and runs in memory
- [x] `start-new` — run a separate `.bc` file with a clean slate (local or remote), for splitting large scaffolds

---

## Project Status

Bytecraft has reached its intended feature set. The core language is complete — variables, loops, conditionals, templates, zip operations, remote execution, and everything in between are all shipped and working.
